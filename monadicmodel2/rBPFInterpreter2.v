...
From bpf.monadicmodel2 Require Import ConcreteState rBPFMonadOp2.
...
Definition step_opcode_alu64 (dst64: val) (src64: val) (dst: reg) (op: nat): M state unit :=
  ...
  | op_BPF_DIV64   =>
    if compl_ne src64 (Vlong Int64.zero) then
      ...
      do _ <- upd_flag (Vint (int_of_flag BPF_ILLEGAL_DIV)); returnM tt (**r we do it for the clightproof , we could omit it later *)
  | op_BPF_OR64    =>
  ...
  | op_BPF_LSH64   =>
    ...
    if compu_lt_32 src32 (Vint (Int.repr 64)) then
      ...
      do _ <- upd_flag (Vint (int_of_flag BPF_ILLEGAL_SHIFT)); returnM tt
  | op_BPF_RSH64   =>
    do src32 <- reg64_to_reg32 src64;
    if compu_lt_32 src32 (Vint (Int.repr 64)) then
  ...
  | op_BPF_NEG64    =>
    if Nat.eqb op 0x87 then
      ...
      do _ <- upd_flag (Vint (int_of_flag BPF_ILLEGAL_INSTRUCTION)); returnM tt
  | op_BPF_MOD64   =>
    if compl_ne src64 (Vlong Int64.zero) then
      do _ <- upd_reg dst (val64_modlu dst64 src64); returnM tt
    else
      do _ <- upd_flag (Vint (int_of_flag BPF_ILLEGAL_DIV)); returnM tt
  | op_BPF_XOR64   =>
  ...
  | op_BPF_ARSH64  =>
    ...
    if compu_lt_32 src32 (Vint (Int.repr 64)) then
      ...
      do _ <- upd_flag (Vint (int_of_flag BPF_ILLEGAL_SHIFT)); returnM tt
  | op_BPF_ALU64_ILLEGAL_INS => do _ <- upd_flag (Vint (int_of_flag BPF_ILLEGAL_INSTRUCTION)); returnM tt
  end.
...
Definition step_opcode_alu32 (dst32: val) (src32: val) (dst: reg) (op: nat): M state unit :=
  ...
  | op_BPF_DIV32   =>
    if comp_ne_32 src32 Vzero then
      ...
      upd_flag (Vint (int_of_flag BPF_ILLEGAL_DIV))
  | op_BPF_OR32   =>
  ...
  | op_BPF_LSH32   =>
    if compu_lt_32 src32 (Vint (Int.repr 32)) then
      ...
      upd_flag (Vint (int_of_flag BPF_ILLEGAL_SHIFT))
  | op_BPF_RSH32   =>
    if compu_lt_32 src32 (Vint (Int.repr 32)) then
  ...
  | op_BPF_NEG32    =>
    if Nat.eqb op 0x84 then
      ...
      upd_flag (Vint (int_of_flag BPF_ILLEGAL_INSTRUCTION))
  | op_BPF_MOD32   =>
    if comp_ne_32 src32 Vzero then
  ...
  | op_BPF_ARSH32  =>
    if compu_lt_32 src32 (Vint (Int.repr 32)) then
      ...
      upd_flag (Vint (int_of_flag BPF_ILLEGAL_SHIFT))
  | op_BPF_ALU32_ILLEGAL_INS => upd_flag (Vint (int_of_flag BPF_ILLEGAL_INSTRUCTION))
  end.
...
Definition step_opcode_branch (dst64: val) (src64: val) (pc: int) (ofs: int) (op: nat) : M state unit :=
  ...
  | op_BPF_CALL =>
    if Nat.eqb op 0x85 then
      ...
      do is_null  <- cmp_ptr32_nullM f_ptr;
        if is_null then
          upd_flag (Vint (int_of_flag BPF_ILLEGAL_CALL))
        else
  ...
  | op_BPF_RET =>
    if Nat.eqb op 0x95 then
      do _ <- upd_flag (Vint (int_of_flag BPF_SUCC_RETURN)); returnM tt
    else
      do _ <- upd_flag (Vint (int_of_flag BPF_ILLEGAL_INSTRUCTION)); returnM tt
...
Definition step_opcode_mem_ld_imm (imm: int) (dst64: val) (dst: reg) (op: nat): M state unit :=
  ...
  | op_BPF_LDDW_high     =>
    do _ <- upd_reg dst (Val.orl dst64 (Val.shll  (Val.longofintu (sint32_to_vint imm)) (sint32_to_vint (Int.repr 32))));
      returnM tt
  | op_BPF_LDX_IMM_ILLEGAL_INS => upd_flag (Vint (int_of_flag BPF_ILLEGAL_INSTRUCTION))
  end.
...
Definition step_opcode_mem_ld_reg (addr: val) (dst: reg) (op: nat): M state unit :=
  ...
  | op_BPF_LDXW      =>
    ...
    do is_null   <- cmp_ptr32_nullM addr_ptr;
      if is_null then
        upd_flag (Vint (int_of_flag BPF_ILLEGAL_MEM))
      else
  ...
  | op_BPF_LDXDW     =>
    ...
    do is_null   <- cmp_ptr32_nullM addr_ptr;
      if is_null then
        ...
        do _ <- upd_reg dst v; returnM tt
  | op_BPF_LDX_REG_ILLEGAL_INS => upd_flag (Vint (int_of_flag BPF_ILLEGAL_INSTRUCTION))
  end.
...
Definition step_opcode_mem_st_imm (imm: val) (addr: val) (op: nat): M state unit :=
  ...
  | op_BPF_ST_IMM_ILLEGAL_INS => upd_flag (Vint (int_of_flag BPF_ILLEGAL_INSTRUCTION))
  end.
...
Definition step_opcode_mem_st_reg (src64: val) (addr: val) (op: nat): M state unit :=
  ...
  | op_BPF_STX_REG_ILLEGAL_INS => upd_flag (Vint (int_of_flag BPF_ILLEGAL_INSTRUCTION))
  end.
...
Definition step: M state unit :=
  ...
  | op_BPF_ILLEGAL_INS => upd_flag (Vint (int_of_flag BPF_ILLEGAL_INSTRUCTION))
  end.
