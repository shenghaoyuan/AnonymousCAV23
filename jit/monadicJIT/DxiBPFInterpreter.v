...
From bpf.jit.monadicJIT Require Import DxJITMonad JITNat DxMonadCommon.
...
Definition get_src64 (x: nat8) (ins: int64): M val64_t :=
  if Int.eq Int.zero (Int.and (nat2int x) int32_8) then
...
Definition step_opcode_alu32 (op: nat8) (pc: int): M unit :=
  ...
  | _ =>
    ...
    do _  <-- magic_function (int_of_nat ofs0);
      upd_pc (Int.add pc (int_of_nat ofs1))
  end.


Definition step_opcode_branch (dst64: val64_t) (src64: val64_t) (pc: int) (ofs: int) (op: nat8) : M unit :=
...
Definition step: M unit :=
  ...
  | op_BPF_ALU32   =>
      step_opcode_alu32 op pc              (**r 0xX4 / 0xXc *)
  | op_BPF_Branch  =>
    do dst64  <-- eval_reg dst;
...
Fixpoint ibpf_interpreter_aux (fuel: nat) {struct fuel}: M unit :=
  match fuel with
  ...
  | S fuel0 =>
    ...
    do pc <-- eval_pc;
      if (Int.ltu pc len) then
        do _ <-- step;
        do f <-- eval_flag;
          if comp_eq_32 f (val_of_flag BPF_OK) then
...
Definition ibpf_interpreter (fuel: nat): M val64_t :=
  do _        <-- ibpf_interpreter_aux fuel;
  do f        <-- eval_flag;
    if comp_eq_32 f (val_of_flag BPF_SUCC_RETURN) then
      do res  <-- eval_reg R0;
        returnM res
    else
      returnM val64_zero.
