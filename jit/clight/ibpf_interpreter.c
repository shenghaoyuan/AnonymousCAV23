...
static __attribute__((always_inline)) inline void jit_alu32_store_flag(struct jit_state* st, unsigned int f)
{
  ...
  _Bool b;
  movw_hi = encode_thumb(11, f, 8U, 4U);
...
static __attribute__((always_inline)) inline void jit_alu32_to_thumb_pass2_aux(struct jit_state* st, unsigned int fuel, unsigned int pc)
{
  ...
  if (fuel == 0U) {
    ...
    if (ins0 == 53504 || ins0 == 56064) {
      ...
      ins = encode_thumb(ofs, ins0, 0U, 8U);
...
static __attribute__((always_inline)) inline void jit_alu32_to_thumb(struct jit_state* st, unsigned int pc)
{
  ...
  len = eval_ins_len(st);
  jit_alu32_to_thumb_pass1(st, len, pc);
  ofs0 = eval_jitted_len(st);
  ofs1 = eval_offset(st);
  add_key_value2(st, pc, ofs0, ofs1);
  jit_alu32_pre(st);
